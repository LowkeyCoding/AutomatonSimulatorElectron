...
var fsm = (function() {
	...
	var domReadyInit = function() {
		...
		$(window).resize(function() {
			container.height($(window).height() - $('#mainHolder h1').outerHeight() - $('#footer').outerHeight() - $('#bulkResultHeader').outerHeight() - $('#resultConsole').outerHeight() - 60 + 'px');
	...
	return {
		...
		load: function() {
			...
			ipcRenderer.on('load-response', (event, arg) => {
				if(!arg.canceled){
					loadSerializedFSM(arg.data);
				}
			});
		},
		...
		save: function() {
			...
			ipcRenderer.send('save', {data: serializedModel});
		}
	};
})().init();
